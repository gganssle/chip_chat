...
# MAGIC decision that is not "call Spark" lives in `chip_chat.databricks.gold`,
# MAGIC where `databricks/tests/test_gold.py` runs it over the real harvest
...
print(f"chunk kinds    {len(gold.KINDS)}")
print(f"chunk fields   {len(gold.FIELDS)}")
...
# MAGIC
# MAGIC Built from `gold.FIELDS` rather than beside it, so that a column added to
# MAGIC the schema arrives here without anyone remembering to add it. The one
...
def spark_type(sql_type):
    """Return the Spark type for one `gold.Field`'s declared SQL type."""
    if sql_type == "STRING":
        return StringType()
    if sql_type == "INT":
        return IntegerType()
    ...
    raise ValueError(
        f"chip_chat.databricks.gold declares a column of type {sql_type!r} and "
...
CONSTRAINTS = {entry.name: entry.constraint for entry in gold.expectations()}
...
# MAGIC
# MAGIC Each renderer is a Python function in `gold.py` that takes one row and
...
_CHUNK = StructType(
    [StructField(entry.name, spark_type(entry.sql_type)) for entry in gold.FIELDS]
)
...
def vocabulary_for(kind):
    ...
    """
    entry = gold.source(kind)
    if entry.vocabulary is None:
        return None
    rows = (
        spark.read.table(catalog.table(silver.LAYER, gold.STREAM, entry.vocabulary))
...
def renderer_for(kind):
    """Return a UDF wrapping the `gold` function that renders one kind."""
    render = getattr(gold, gold.source(kind).renderer)
    columns = gold.source(kind).columns
    ...
    def _render(*values):
        ...
        # out unchanged; `gold` deliberately does not parse a timestamp it
        ...
        # struct carries four fields and `gold.FIELDS` declares two, and a
        ...
        row[gold.ALLERGENS] = list(row[gold.ALLERGENS] or []) or None
        row[gold.COLUMN_HEADERS] = list(row[gold.COLUMN_HEADERS] or []) or None
        row["cells"] = list(row["cells"] or []) or None
        row[gold.CITATION] = [
            {"harvested_at": one["harvested_at"], "source_url": one["source_url"]}
            for one in (row[gold.CITATION] or ())
        ] or None
        return row
...
def chunks_of(kind):
    """Return the chunks one silver table becomes, as a frame of `gold.FIELDS`."""
    entry = gold.source(kind)
    frame = spark.read.table(
        catalog.table(silver.LAYER, gold.STREAM, entry.table)
    ).select(*[F.col(column) for column in entry.columns])
    return (
        frame.withColumn("_chunk", renderer_for(kind)(*entry.columns))
        .select("_chunk.*")
        .withColumn(gold.CHUNKED_AT, F.current_timestamp())
...
@dp.materialized_view(
    name=catalog.table(gold.LAYER, gold.STREAM, gold.CHUNK_TABLE),
    comment=gold.CHUNK_COMMENT,
    table_properties={
        "chip_chat.stream": gold.STREAM,
...
def _corpus_chunks():
    frames = [chunks_of(kind) for kind in gold.KINDS]
    combined = frames[0]
