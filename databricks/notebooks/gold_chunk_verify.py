...
# MAGIC `databricks/tests/test_gold_chunks.py` runs the chunker over the recorded
...
# MAGIC other half, run as the `chip-chat-gold-chunk-verify` job. SUCCESS means the
...
# MAGIC    holds rows of every kind, and its columns are exactly `gold_chunks.FIELDS` —
...
# MAGIC `gold_chunks.EMBEDDING_CHARACTER_BUDGET`. That is deliberate and is the whole of
...
# MAGIC
# MAGIC Run it after the pipeline, from the `chip-chat-gold-chunk-verify` job. It
...
dbutils.widgets.text("lib_path", "", "Workspace directory holding gold_chunks.py")
...
CHUNKS = catalog.table(gold_chunks.LAYER, gold_chunks.STREAM, gold_chunks.CHUNK_TABLE)
...
declared = [entry.name for entry in gold_chunks.FIELDS]
check(
...
by_kind = {
    row[gold_chunks.KIND]: row["count"]
    for row in chunks.groupBy(gold_chunks.KIND).count().collect()
}
for kind in gold_chunks.KINDS:
    counts[kind] = by_kind.get(kind, 0)
    check(counts[kind] > 0, f"{kind}: {counts[kind]} chunks")

check(
    chunks.select(gold_chunks.CHUNK_ID).distinct().count() == total,
...
for expectation in gold_chunks.expectations():
    ...
    if offenders:
        violations.select(
            gold_chunks.CHUNK_ID, gold_chunks.KIND, gold_chunks.SOURCE_URL
...
tables = spark.table(catalog.table(silver.LAYER, gold_chunks.STREAM, "document_tables"))
extracted = tables.count()
rendered = chunks.where(f"{gold_chunks.KIND} = '{gold_chunks.NUTRITION_ROW}'").count()
...
blocks = spark.table(
    catalog.table(silver.LAYER, gold_chunks.STREAM, "document_blocks")
).count()
block_chunks = chunks.where(
    f"{gold_chunks.KIND} = '{gold_chunks.DOCUMENT_BLOCK}'"
).count()
...
items = spark.table(catalog.table(silver.LAYER, gold_chunks.STREAM, "menu_items")).count()
item_chunks = chunks.where(f"{gold_chunks.KIND} = '{gold_chunks.MENU_ITEM}'").count()
...
# MAGIC
# MAGIC `gold_chunks.EMBEDDING_CHARACTER_BUDGET` is the one number in this layer and it
...
oversize = chunks.where(
    f"{gold_chunks.CHARACTER_COUNT} > {gold_chunks.EMBEDDING_CHARACTER_BUDGET}"
)
counts["over_budget"] = oversize.count()
print(
    f"  note   {counts['over_budget']} chunks exceed "
    f"{gold_chunks.EMBEDDING_CHARACTER_BUDGET} characters. Nothing was split."
)
if counts["over_budget"]:
    oversize.select(
        gold_chunks.KIND,
        gold_chunks.HEADING,
        gold_chunks.CHARACTER_COUNT,
        gold_chunks.SOURCE_URL,
    ).orderBy(gold_chunks.CHARACTER_COUNT, ascending=False).show(10, truncate=False)
...
sample = (
    chunks.selectExpr(
        ...
        f"row_number() OVER (PARTITION BY {gold_chunks.KIND} "
        f"ORDER BY {gold_chunks.CHUNK_ID}) AS _rank",
    )
    .where(f"_rank <= {max(1, SAMPLE // len(gold_chunks.KINDS))}")
    .drop("_rank")
    .orderBy(gold_chunks.KIND, gold_chunks.CHUNK_ID)
    .limit(SAMPLE)
)
for row in sample.collect():
    print(f"--- {row[gold_chunks.KIND]}  {row[gold_chunks.CHUNK_ID][:12]}")
    print(f"    heading: {row[gold_chunks.HEADING]}")
    print(
        f"    source:  {row[gold_chunks.SOURCE_URL]}  ({row[gold_chunks.HARVESTED_AT]})"
    )
    print(f"    {row[gold_chunks.TEXT]}")
    print()
...
if failures:
    raise AssertionError(
        f"{len(failures)} gold chunk claims failed:\n"
