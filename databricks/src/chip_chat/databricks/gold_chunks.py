...
lets this file decide what comes out. ``databricks/tests/test_gold_chunks.py``
runs the renderers over the real harvest fixtures without a cluster, which is
what makes the issue's two required tests tests rather than intentions.

**Gold is two things and they are two modules.** :mod:`chip_chat.databricks.gold`
is #36's four personalization marts, computed over the *synthetic* stream; this
is #35's corpus chunks, rendered from the *harvested* one. They share a
medallion layer and nothing else -- different schema, different pipeline,
different stream, and the boundary between the two streams is the one RFC-001
§04 is least willing to see blurred. Two modules named for what they build
rather than one named for the layer they land in is the arrangement that keeps
:data:`STREAM` a constant here and ``gold_synthetic`` a constant there. The
schema this one fixes is restated in :mod:`chip_chat.search.chunks` for #48's
index, and ``search/tests/test_chunk_contract.py`` holds the two together.
...
for the reason the module docstring gives. ``test_gold_chunks.py`` asserts they agree.
"""
...
CHUNK_COMMENT: Final = (
    ...
    "window size anywhere in chip_chat.databricks.gold_chunks — RFC-001 §08 says a "
...
constant here is copied, and asserted equal to it in ``test_gold_chunks.py``.
"""
...
class Chunk:
    ...
    Every field of :data:`FIELDS` appears here, and ``test_gold_chunks.py`` asserts
...
class Source:
    ...
    Attributes:
        ...
        renderer: The module-level function that turns one row into one chunk.
            Named here so ``test_gold_chunks.py`` can assert that every kind has one
...
class Expectation:
    ...
    imported from it, for the reason the module docstring gives. ``test_gold_chunks``
