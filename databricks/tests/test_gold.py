...
`chip_chat.databricks.gold` is two things and this file checks them differently.
...
MODULE = REPO / "databricks" / "src" / "chip_chat" / "databricks" / "gold.py"
...
def every_chunk(
    ...
    chunks: list[gold.Chunk] = []
    chunks += [gold.menu_item_chunk(row, allergen_names) for row in menu_items]
    chunks += [gold.policy_section_chunk(row) for row in _POLICY_SECTIONS]
    chunks += [gold.faq_entry_chunk(row) for row in _FAQ_ENTRIES]
    chunks += [gold.allergen_caveat_chunk(row) for row in caveats]
    chunks += [gold.document_block_chunk(row) for row in _DOCUMENT_BLOCKS]
    chunks += [gold.nutrition_row_chunk(row) for row in nutrition_rows]
    return tuple(chunks)
...
def _fixed_window(text: str, size: int, overlap: int = 0) -> tuple[str, ...]:
    ...
    It lives in the test file rather than in `chip_chat.databricks.gold`
...
def test_every_chunk_has_a_citable_source(corpus: tuple[gold.Chunk, ...]) -> None:
    ...
    """
    assert {chunk.kind for chunk in corpus} == set(gold.KINDS), (
...
def test_the_citation_is_an_expectation_and_not_only_a_habit() -> None:
    ...
    constraint = {entry.name: entry.constraint for entry in gold.expectations()}
    assert constraint["carries_its_citation"] == (
        f"{gold.SOURCE_URL} IS NOT NULL AND {gold.HARVESTED_AT} IS NOT NULL"
    )
...
def test_the_chunk_dataclass_and_the_declared_schema_are_the_same_schema() -> None:
    ...
    """
    assert [entry.name for entry in gold.FIELDS] == [
        entry.name for entry in dataclass_fields(gold.Chunk)
    ]


def test_a_chunk_renders_to_exactly_the_declared_columns(
    corpus: tuple[gold.Chunk, ...],
) -> None:
    for chunk in corpus:
        assert list(chunk.as_row()) == [entry.name for entry in gold.FIELDS]
...
def test_the_issue_asks_for_these_fields_by_name() -> None:
    ...
    names = {entry.name for entry in gold.FIELDS}
    for required in (
        gold.ITEM_ID,
        gold.CATEGORY,
        gold.ALLERGENS,
...
def test_the_comparative_questions_are_filters_and_not_paraphrases() -> None:
    ...
    filters = set(gold.filterable())
    assert gold.CALORIES in filters
    assert gold.ALLERGENS in filters
    assert gold.CATEGORY in filters
...
def test_every_facetable_field_is_also_filterable() -> None:
    ...
    assert set(gold.facetable()) <= set(gold.filterable())


def test_the_free_text_field_is_not_facetable() -> None:
    """Faceting a sentence buys a histogram of sentences."""
    assert not gold.field(gold.TEXT).facetable
    assert not gold.field(gold.TEXT).filterable
...
def test_the_citation_fields_come_back_with_a_hit() -> None:
    ...
    retrievable = set(gold.retrievable())
    assert gold.SOURCE_URL in retrievable
    assert gold.HARVESTED_AT in retrievable
    assert gold.CHUNK_ID in retrievable
...
def test_a_field_says_which_kinds_populate_it() -> None:
    ...
    a query."""
    assert gold.field(gold.ITEM_ID).kinds == (gold.MENU_ITEM,)
    assert gold.field(gold.TEXT).universal
    assert gold.field(gold.SOURCE_URL).universal


def test_columns_for_a_kind_are_its_own_plus_the_universal_ones() -> None:
    names = [entry.name for entry in gold.columns_for(gold.NUTRITION_ROW)]
    assert gold.COLUMN_HEADERS in names
    assert gold.CHUNK_ID in names
    assert gold.ITEM_ID not in names


def test_columns_for_refuses_a_kind_nothing_produces() -> None:
    with pytest.raises(ValueError, match="unknown chunk kind"):
        list(gold.columns_for("PARAGRAPH"))
...
def test_a_menu_chunk_carries_its_nutrition_as_a_column_and_not_only_as_prose(
    ...
    chunk = gold.menu_item_chunk(guacamole, allergen_names)
...
def test_a_composed_item_says_its_calories_are_one_component(
    ...
    chunk = gold.menu_item_chunk(composed, allergen_names)
...
def test_the_two_allergen_silences_do_not_read_the_same(
    ...
    published = gold.menu_item_chunk({**base, "allergen_disclosure": "PUBLISHED"})
    silent = gold.menu_item_chunk({**base, "allergen_disclosure": "NOT_PUBLISHED"})
...
def test_an_allergen_renders_under_its_published_label(
    ...
    chunk = gold.menu_item_chunk(marked, allergen_names)
...
def test_a_menu_row_with_no_item_id_is_refused(
    ...
    with pytest.raises(ValueError, match="published item_id"):
        gold.menu_item_chunk({"name": "Nameless"}, allergen_names)
...
def test_a_policy_chunk_names_the_document_it_is_a_section_of() -> None:
    ...
    chunk = gold.policy_section_chunk(_POLICY_SECTIONS[1])
...
def test_a_section_that_names_no_document_is_refused() -> None:
    with pytest.raises(ValueError, match="the document it is a section of"):
        gold.policy_section_chunk({"position": 0, "text": "Points expire."})
...
def test_a_faq_question_and_its_answer_are_one_chunk() -> None:
    ...
    chunk = gold.faq_entry_chunk(_FAQ_ENTRIES[0])
...
def test_a_faq_row_with_no_question_is_refused() -> None:
    with pytest.raises(ValueError, match="published question"):
        gold.faq_entry_chunk({"category": "Rewards Program", "answer": "Yes."})
...
def test_the_cross_contact_caveat_survives_into_a_chunk(
    ...
    texts = " ".join(gold.allergen_caveat_chunk(row).text for row in caveats)
...
def test_a_heading_the_text_already_opens_with_is_not_said_twice(
    ...
    text = gold.allergen_caveat_chunk(repeated).text
...
def test_a_heading_the_text_does_not_open_with_is_kept() -> None:
    ...
    with no subject."""
    chunk = gold.allergen_caveat_chunk(
        {
            "position": 9,
...
def test_an_empty_caveat_is_refused() -> None:
    with pytest.raises(ValueError, match="is not a caveat"):
        gold.allergen_caveat_chunk({"position": 0, "text": "  "})
...
def test_a_document_block_keeps_every_source_that_published_it() -> None:
    ...
    chunk = gold.document_block_chunk(_DOCUMENT_BLOCKS[0])
...
def test_a_document_block_is_keyed_on_the_digest_silver_gave_it() -> None:
    ...
    assert gold.document_block_chunk(block).chunk_id == gold.chunk_id(
        gold.DOCUMENT_BLOCK, block["block_sha256"]
    )


def test_a_block_with_no_digest_is_refused() -> None:
    with pytest.raises(ValueError, match="block_sha256"):
        gold.document_block_chunk({"heading": "Our Food", "text": "Real ingredients."})
...
def test_a_hole_in_a_row_is_named_rather_than_closed_up() -> None:
    ...
    different route."""
    chunk = gold.nutrition_row_chunk(
        {
            "content_sha256": "0" * 64,
            "table_index": 0,
            "row_index": 1,
...
def test_a_row_with_no_cells_is_refused() -> None:
    with pytest.raises(ValueError, match="is not a row"):
        gold.nutrition_row_chunk({"column_headers": [], "cells": []})


def test_a_nutrition_chunk_cites_the_page_it_was_read_from(
    nutrition_rows: tuple[dict[str, Any], ...],
) -> None:
    chunk = gold.nutrition_row_chunk(nutrition_rows[0])
...
def test_a_chunk_id_names_what_the_chunk_is_about_and_not_what_it_says() -> None:
    ...
    first = gold.menu_item_chunk(before)
    second = gold.menu_item_chunk(after)
...
def test_two_kinds_with_the_same_key_are_two_chunks() -> None:
    ...
    uncitable."""
    assert gold.chunk_id(gold.ALLERGEN_CAVEAT, 0) != gold.chunk_id(gold.POLICY_SECTION, 0)
...
def test_a_missing_key_part_is_not_the_empty_string() -> None:
    ...
    assert gold.chunk_id(gold.POLICY_SECTION, None) != gold.chunk_id(
        gold.POLICY_SECTION, ""
    )
...
def test_an_id_over_the_kind_alone_is_refused() -> None:
    ...
    with pytest.raises(ValueError, match="the kind alone is not one"):
        gold.chunk_id(gold.MENU_ITEM)


def test_an_unknown_kind_has_no_id() -> None:
    with pytest.raises(ValueError, match="unknown chunk kind"):
        gold.chunk_id("PARAGRAPH", "CMG-1")
...
def test_identity_normalises_the_way_silver_does() -> None:
    ...
    for text in ("Steak\xa0 Burrito\n", "  Chips  and   Guacamole ", "Café"):
        assert gold.normalise(text) == silver.normalise(text)
...
def _code_without_prose(source: str) -> str:
    ...
    matters for exactly this test: `gold.CHUNK_COMMENT` contains the sentence
    "there is no window size anywhere in chip_chat.databricks.gold", and a
...
def test_the_budget_reports_and_does_not_split(corpus: tuple[gold.Chunk, ...]) -> None:
    """Every fixture chunk is comfortably under it, and nothing checked."""
    assert all(
        chunk.character_count <= gold.EMBEDDING_CHARACTER_BUDGET for chunk in corpus
    )
    assert gold.EMBEDDING_CHARACTER_BUDGET > 0
...
def test_a_long_published_section_stays_one_chunk() -> None:
    ...
    chunk = gold.policy_section_chunk(
        {
            "document_id": "rewards-terms",
            "document_kind": "TERMS",
    ...
    assert chunk.character_count > gold.EMBEDDING_CHARACTER_BUDGET
...
def test_chunks_are_harvested_and_there_is_no_synthetic_corpus() -> None:
    ...
    assert gold.STREAM in catalog.STREAMS
    assert gold.STREAM == "harvested"
    assert all(gold.source(kind).table for kind in gold.KINDS)


def test_the_schema_name_is_the_one_terraform_created() -> None:
    assert gold.schema_name() == catalog.schema("gold", "harvested").name


def test_the_allergen_states_are_the_harvest_s_own() -> None:
    assert gold.CONTAINS == AllergenStatus.CONTAINS
    assert gold.NOT_PUBLISHED == AllergenStatus.NOT_PUBLISHED
    assert set(gold.DISCLOSURES) == {member.value for member in AllergenDisclosure}
...
def test_the_expectation_record_is_the_shape_silver_uses() -> None:
    ...
    assert [entry.name for entry in dataclass_fields(gold.Expectation)] == [
        entry.name for entry in dataclass_fields(silver.Expectation)
    ]


def test_every_kind_is_produced_by_exactly_one_source() -> None:
    assert [entry.kind for entry in gold.SOURCES] == list(gold.KINDS)
    assert len({entry.kind for entry in gold.SOURCES}) == len(gold.SOURCES)
...
def test_every_renderer_named_by_a_source_exists_and_is_used() -> None:
    ...
    named = {entry.renderer for entry in gold.SOURCES}
    for name in named:
        assert callable(getattr(gold, name)), name
    exported = {
        name
        for name in gold.__all__
        if name.endswith("_chunk") and callable(getattr(gold, name))
    }
...
def test_every_expectation_names_what_is_true_rather_than_what_went_wrong() -> None:
    ...
    for entry in gold.expectations():
...
def test_a_per_kind_expectation_passes_for_every_other_kind() -> None:
    ...
    kind."""
    for entry in gold.expectations():
        if f"{gold.KIND} <> " in entry.constraint:
...
def test_the_nutrition_expectation_is_the_first_required_test_as_a_constraint() -> None:
    constraint = {entry.name: entry.constraint for entry in gold.expectations()}
    assert (
        f"size({gold.COLUMN_HEADERS}) = size(cells)" in constraint["keeps_the_row_whole"]
    )
...
def test_the_pipeline_never_reads_the_synthetic_stream(notebook: str) -> None:
    ...
    assert "gold.STREAM" in code


def test_the_chunk_struct_is_built_from_the_declared_schema(notebook: str) -> None:
    """Rather than beside it, so a column added to `gold.FIELDS` arrives in the
    pipeline without anyone remembering to add it there too."""
    code = "\n".join(
        line for line in notebook.splitlines() if not line.startswith("# MAGIC")
    )
    assert "for entry in gold.FIELDS" in code
...
def test_the_citation_array_is_narrowed_by_name(notebook: str) -> None:
    """Silver's citation struct carries four fields and `gold.FIELDS` declares
