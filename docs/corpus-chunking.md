...
`databricks/notebooks/gold_chunk.py`, `databricks/notebooks/gold_verify.py` and
...
> described, and including the fixed-window chunker in `test_gold.py` that the
> same assertions are run over and required to **fail**. The pipeline has
...
So `chip_chat.databricks.gold` has no window size, no overlap, no target length
...
`gold.STREAM` is a constant rather than a loop variable, so there is no code
...
`gold.FIELDS` is the schema, and it is a tuple of records rather than a
...
`gold.nutrition_row_chunk` cannot produce that chunk. Not because it is careful
...
substituted*. So `databricks/tests/test_gold.py` holds a real fixed-window
...
| --- | --- | --- |
| `test_no_nutrition_table_is_split_across_a_chunk_boundary` | `gold.nutrition_row_chunk` | every row whole |
...
`gold.EMBEDDING_CHARACTER_BUDGET` is the one number in this layer and it splits
nothing. `gold_verify.py` reports the chunks over it and the pipeline does not
look at it at all.
...
> pipeline runs, `gold_verify.py` prints a deterministic twenty and this section
...
2. **A heading stated twice.** Chipotle's `GLUTEN INTOLERANCE & CELIAC DISEASE`
   ...
   `gold._headed` now skips the prefix when the text already opens with it, and
...
`gold_verify.py`, run as the `chip-chat-gold-verify` job. It reads and never
writes.
...
1. `chip_chat.gold_harvested.corpus_chunks` exists, holds rows of all six kinds,
   and its columns are exactly `gold.FIELDS` in order.
...
The argument for a third pipeline rather than a third half of the second is the
one `databricks_silver.tf` makes about bronze, one layer up, and its second half
applies harder: gold is now the layer whose logic changes — a renderer's
wording, a new chunk kind, a metadata field #48 turns out to need — and iterating
on it should not involve recomputing twenty-four silver tables to find out
whether a sentence reads better.

## 9. What this hands to #48

- One table, `chip_chat.gold_harvested.corpus_chunks`, and one schema,
  `gold.FIELDS`, with retrievability, filterability and facetability already
...
- **It has not been run.** See the status note at the top. Two of the three
  ...
  third is §6, done against recorded fixtures. Until `gold_verify` has returned
