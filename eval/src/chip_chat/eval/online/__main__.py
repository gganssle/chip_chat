...
Without either flag it reads live turns from a capture file and runs the loop
over them. A capture is what a backend adapter wrote; see
``eval/online/README.md`` for the shape and for why this reads a file rather than
querying a backend directly.
...
from chip_chat.eval.online.monitors import MONITORS, evaluate
...
def main(argv: list[str] | None = None) -> int:
    ...
    Returns:
        ...
        unaccounted, a drill's monitor did not fire, or a capture cannot be
        read.
    """
    ...
    if args.check or args.capture is None:
...
def _live(args: argparse.Namespace, policy: SamplingPolicy) -> int:
    """Run the loop over a capture of live turns."""
    try:
